...
parser.add_argument("--decompressed_folder_name", type=str, default='generated_data_decompressed')
...
def postprocess_df(df):
    ...
    df_partitions = df_copy.groupby('partition', as_index=False)['tweet_id'].apply(len).rename(
        columns={'tweet_id': 'cluster_size'}
    )
    df_partitions.index = df_partitions.partition
    df_partitions.drop(columns=['partition'], inplace=True)
    sorted_d = dict( sorted(df_partitions.to_dict()['cluster_size'].items(), key=operator.itemgetter(1),reverse=True))
